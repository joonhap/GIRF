...
#pragma omp parallel num_threads(R) copyin(dist, birth_data, birth_data_first_year, birth_data_last_year, pop_data, pop_data_first_year, pop_data_last_year, dist_mean, pop_mean, cityindex)
  ...
  //  std::string data_spec = "G" + std::to_string(G) + "rep" + std::to_string(rep); COMMENTED OUT FOR RUNNING WITH EXACT PARAMETER SET
  std::string data_spec = "G" + std::to_string(G) + "rep" + std::to_string(rep) + "exact";
  ...
  if (!run_IF2) { // always run_IF2=false when running this script
    /* This region was commented in order to run the filter with the exact parameter set to generate a plot in the paper. COMMENT START
    ...
    // sobj.imfilter(initstates, N, R, J, lestimate, state_mean, q10, q50, q90, square, f1_mean, ESS, theta_point, weighted_interaction, max_lookahead, island_interact_condition, initial_seed); // run filter COMMENTED OUT FOR RUNNING WITH EXACT PARAMETER SET
    sobj.imfilter(initstates, N, R, J, lestimate, state_mean, q10, q50, q90, square, f1_mean, ESS, sobj.theta, weighted_interaction, max_lookahead, island_interact_condition, initial_seed); // run filter
    M = 1;
  }

  ...
  if (run_IF2) {
	...
	for (int i=0; i<R; i++)
	  for(int j=0; j<J; j++) {
	    ...
	    for (int k=0; k<ncity; k++) {
	      unif = uniform(rng);
	      initThetaSwarm[i][j][17+3*k] = log((0.02+(0.06-0.02)*unif)/ (1.0 - (0.02+(0.06-0.02)*unif))); // S proportion
	      unif = uniform(rng);
	      initThetaSwarm[i][j][17+3*k+1] = log((0.0001+(0.0005-0.0001)*unif) / (1.0 - (0.0001+(0.0005-0.0001)*unif))); // E proportion
	      unif = uniform(rng);
	      initThetaSwarm[i][j][17+3*k+2] = log((0.0001+(0.0005-0.0001)*unif) / (1.0 - (0.0001+(0.0005-0.0001)*unif))); // I proportion
	    }
	  }
      }
      else if (continued) {
...
#pragma omp parallel num_threads(R) copyin(dist, birth_data, birth_data_first_year, birth_data_last_year, pop_data, pop_data_first_year, pop_data_last_year, dist_mean, pop_mean, cityindex)
      ...
      sobj.imfilter(initstates, N, R, J, lestimate, state_mean, q10, q50, q90, square, f1_mean, ESS, initThetaSwarm, perturb_size, M, coolschedule, thetaTrajectory, finalThetaSwarm, weighted_interaction, max_lookahead, island_interact_condition, initial_seed);
